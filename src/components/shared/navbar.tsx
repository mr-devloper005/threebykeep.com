...
export function Navbar() {
  ...
  const primaryTaskKey = siteContent.taskPriority.primary
  const secondaryTaskKey = siteContent.taskPriority.secondary
  const featuredPrimary = navigation.find((task) => task.key === primaryTaskKey) || navigation[0]
  const featuredSecondary = navigation.find((task) => task.key === secondaryTaskKey && task.key !== featuredPrimary?.key) || navigation.find((task) => task.key !== featuredPrimary?.key)
  const featuredNavigation = [featuredPrimary, featuredSecondary].filter((task): task is NonNullable<typeof task> => Boolean(task))
  const utilityNavigation = navigation.filter((task) => !featuredNavigation.some((item) => item.key === task.key))
  const mobileNavigation = [...featuredNavigation, ...utilityNavigation].map((task) => ({
    name: task.label,
    href: task.route,
    icon: taskIcons[task.key] || LayoutGrid,
    featured: featuredNavigation.some((item) => item.key === task.key),
  }))
  const primaryTask = featuredPrimary || SITE_CONFIG.tasks.find((task) => task.key === recipe.primaryTask) || navigation[0]
        ...
        <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden min-w-0 flex-1 items-center justify-center lg:flex">
            <div className={cn('flex w-full max-w-xl items-center gap-3 rounded-full px-4 py-3', palette.search)}>
              ...
              <span className="text-sm">Find bookmarks, articles, and reference files</span>
          ...
          <div className={palette.mobile}>
            <div className="space-y-2 px-4 py-4">
              <div className={cn('mb-3 flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium', palette.search)}>
                <Search className="h-4 w-4" />
                Find bookmarks, articles, and resources
              </div>
              {mobileNavigation.map((item) => {
                const isActive = pathname.startsWith(item.href)
                return (
                  <Link key={item.name} href={item.href} onClick={() => setIsMobileMenuOpen(false)} className={cn('flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition-colors', isActive ? 'bg-foreground text-background' : item.featured ? palette.post : 'opacity-85')}>
        ...
        <div className={style.mobile}>
          <div className="space-y-2 px-4 py-4">
            ...
            {mobileNavigation.map((item) => {
              const isActive = pathname.startsWith(item.href)
              return (
                <Link key={item.name} href={item.href} onClick={() => setIsMobileMenuOpen(false)} className={cn('flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition-colors', isActive ? style.active : item.featured ? style.idle : 'text-muted-foreground hover:bg-muted')}>
                  <item.icon className="h-5 w-5" />
                  {item.name}
                </Link>
              )
